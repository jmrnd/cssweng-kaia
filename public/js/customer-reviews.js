...
const likeIconFill = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up-fill" viewBox="0 0 16 16">
...
const likeIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-hand-thumbs-up" viewBox="0 0 16 16">
...
function renderAverageRating() {
    ...
    const averageRating = parseInt(averageRatingText.match(/\d/)[0], 10);
    const fullStars = averageRating;
    const starsLeft = maxRating - averageRating;
...
function renderLike(review) {
    const likeElement = review.querySelector(".review-likes");
    const likeButton = review.querySelector(".review-like-button");
    const likeNumber = parseInt(review.querySelector("#reviewLike"));

    const isLiked = likeButton.classList.contains("liked");

    if (isLiked) {
        likeButton.innerHTML += likeIconFill;
    } else {
...
function toggleLike(likeElement) {
    const likeButton = likeElement.querySelector(".review-like-button");
    const likeText = likeElement.querySelector("#reviewLike");
    const isLiked = likeButton.classList.contains("liked");
    let likes = parseInt(likeText.innerText);
    if (isLiked) {
